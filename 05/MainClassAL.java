...
class MainClassAL {
	public static void main(String args[]) {
		ArrayList<Animal> animals = new ArrayList<Animal>(); // создаём массив объектов ArrayList
		...
		for(int i = 0; i < animals.size(); i++) animals.get(i).walk();
		System.out.println();
		System.out.println("And than...");
		// and makes sounds
		for(int i = 0; i < animals.size(); i++) animals.get(i).voice();		
		
		System.out.println();
		System.out.println("But near this place hunters are shooting ducks. And a loud shoot brings animals to panic.");
		// and makes sounds
		for(int i = 0; i < animals.size(); i++) animals.get(i).run();		
		
		System.out.println();
		System.out.println("There are some troubles it meets.");
		System.out.println();
		for(int j = 8; j <= 35; j+=7)
		{
			System.out.println();
			System.out.println("There is barrier " + ((double)j / 10) + " meters height, and animals need to jump over it. Let's go!");
			for(int i = 0; i < animals.size(); i++) animals.get(i).jump(j);
						
			System.out.println();
			System.out.println("But there was an moat behind barrier. And animal is needed to swim. If it can.");
			for(int i = 0; i < animals.size(); i++) animals.get(i).swim();			
		}
		System.out.println();
